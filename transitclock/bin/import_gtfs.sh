...
java -Xmx512m -Dtransitclock.core.agencyId=$AGENCYID -Dtransitclock.configFiles="$CONFIG_DIR/transitclock.properties" -Dtransitclock.logging.dir=/usr/local/transitclock/logs/ -Dlogback.configurationFile="$SOURCE_DIR/transitclock/src/main/resouces/logbackGtfs.xml" -cp "$LIB_DIR/Core.jar" org.transitclock.applications.GtfsFileProcessor -gtfsUrl $GTFS_URL -maxTravelTimeSegmentLength 100
