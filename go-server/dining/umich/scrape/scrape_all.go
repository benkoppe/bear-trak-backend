package scrape

import (
	"fmt"
	"log"
	"sync"
	"time"

	"github.com/benkoppe/bear-trak-backend/go-server/dining/umich/static"
	"github.com/benkoppe/bear-trak-backend/go-server/utils"
)

type Cache = *utils.Cache[map[*static.Eatery][]Eatery]

func InitCache(baseURL string) Cache {
	return utils.NewCache(
		"diningScrape",
		time.Hour*24,
		func() (map[*static.Eatery][]Eatery, error) {
			staticEateries := static.GetEateries()
			return fetchAll(baseURL, staticEateries)
		},
	)
}

func fetchAll(baseURL string, eateries []static.Eatery) (map[*static.Eatery][]Eatery, error) {
	fetchedEateries := make(map[*static.Eatery][]Eatery)
	mu := sync.Mutex{}
	wg := sync.WaitGroup{}

	total := len(eateries) * 7
	progressCh := make(chan struct{}, total)

	// progress printer
	go func() {
		done := 0
		for range progressCh {
			done++
			fmt.Printf("Progress: %d/%d (%.1f%%)\n", done, total, 100*float64(done)/float64(total))
		}
	}()

	semaphore := make(chan struct{}, 5) // global limit
	scraper := utils.NewBrowserScraper()
	defer scraper.Close()

	for i := range eateries {
		wg.Add(1)
		go func(e *static.Eatery) {
			defer wg.Done()

			eateryURL, err := utils.ExtendURL(baseURL, e.ScrapePath)
			if err != nil {
				log.Printf("error extending url for eatery %d: %v", e.ID, err)
				return
			}

			eateryWeek, err := fetchEateryWeekConcurrent(*eateryURL, semaphore, scraper, progressCh)
			if err != nil {
				log.Printf("error fetching eatery week for eatery %d: %v", e.ID, err)
				return
			}

			mu.Lock()
			fetchedEateries[e] = eateryWeek
			mu.Unlock()
		}(&eateries[i])
	}

	wg.Wait()
	close(progressCh)
	log.Printf("cache refresh completed: %d/%d eateries fetched successfully", len(fetchedEateries), len(eateries))
	return fetchedEateries, nil
}

// unused with new chromedp approach -- reverted back to old fetchEateryWeek

func fetchEateryWeekConcurrent(eateryURL string, semaphore chan struct{}, scraper *utils.BrowserScraper, progressCh chan<- struct{}) ([]Eatery, error) {
	now := time.Now()
	eateryWeek := make([]Eatery, 7)

	var wg sync.WaitGroup
	var errMu sync.Mutex
	var firstErr error

	for dayOffset := range [7]int{} {
		wg.Add(1)
		go func(dayOffset int) {
			defer wg.Done()

			semaphore <- struct{}{}
			defer func() { <-semaphore }()

			date := now.AddDate(0, 0, dayOffset)
			eatery, err := fetchEatery(scraper, eateryURL, date)
			if err != nil {
				errMu.Lock()
				if firstErr == nil {
					firstErr = fmt.Errorf("failed to fetch eatery for %s: %w", date.Format("2006-01-02"), err)
				}
				errMu.Unlock()
			} else {
				eateryWeek[dayOffset] = *eatery
			}

			// notify overall progress
			progressCh <- struct{}{}
		}(dayOffset)
	}

	wg.Wait()
	if firstErr != nil {
		return nil, firstErr
	}

	return eateryWeek, nil
}
